        ...
        if i_iter not in all_res.keys():
            ...
            for index in tqdm(range(len(test_loader))):
                ...
                with torch.no_grad():
                    ...
                    output = np.argmax(output, axis=2)
                label = label.numpy()[0]
                hist_ = fast_hist(label.flatten(), output.flatten(), cfg.NUM_CLASSES)
                inters_over_union_classes_ = per_class_iu(hist_)
                computed_miou_ = round(np.nanmean(inters_over_union_classes_) * 100, 2)
                print('miou:{}'.format(inters_over_union_classes_))
                
                with open(str(i_iter)+'mIoU.txt','a+') as f:
                    f.write(" ".join(map(str,inters_over_union_classes_)))
                    f.write('\n')
                f.close()
